...
RESOURCE(res_dissociation,
         "title=\"Patient dissociation\";rt=\"application/json\"",
         NULL,
         res_post_handler,
         NULL,
         NULL);
...
static void res_post_handler(coap_message_t *request, coap_message_t *response, uint8_t *buffer, uint16_t preferred_size, int32_t *offset) {
    ...
    //  PAYLOAD PARSING
        ...
        //  Missing payload
        if(payload_len <= 0) {
            LOG_ERR("Bad dissociation request: no payload attached\n");
        ...
        if(payload_len >= REST_MAX_CHUNK_SIZE) {
            LOG_ERR("Bad dissociation request: payload too large\n");
        ...
        LOG_INFO("Dissociation request received: %s\n", (char*)buffer);
        
        if(parse_dissociation_payload(buffer, &patient_id) != 0) {
            coap_set_status_code(response, BAD_REQUEST_4_00);
            return;
        }
    ...
    LOG_INFO("Dissociation request of patient %i . . .\n", patient_id);
    ...
    switch (remove_outcome) {
        ...
        case RESULT_INVALID_INPUT:
            LOG_ERR("Bad dissociation request patient %i: invalid input data\n", patient_id);
            coap_set_status_code(response, BAD_REQUEST_4_00);
            return;

        case RESULT_PATIENT_NOT_ASSOCIATED:
            LOG_ERR("Bad dissociation request patient %i: was not associated to this nurse\n", patient_id);
            coap_set_status_code(response, BAD_REQUEST_4_00);
            return;
        ...
        default:
            LOG_ERR("Bad dissociation request patient %i: uknown error\n", patient_id);
            coap_set_status_code(response, BAD_REQUEST_4_00);
            return;
    }
